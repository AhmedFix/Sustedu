...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Books</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('books.create') }}" class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-2/12 px-4 py-3">Book Type</div>
                <div class="w-2/12 px-4 py-3">Subject</div>
                <div class="w-2/12 px-4 py-3">Author</div>
                <div class="w-1/12 px-4 py-3 text-center">Poster</div>
                <div class="w-2/12 px-4 py-3 text-right">Action</div>
            </div>
            @foreach ($books as $book)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $book->id }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $book->name }}</div>
                    ...
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $book->subject->name ?? '' }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $book->author ?? '' }}</div>
                    <div class="w-1/12 px-4 py-3 text-sm text-gray-600 text-center tracking-tight">
                        <img class="w-15 h-20 sm:w-16 sm:h-25 rounded" src="{{ asset('images/books/' .$book->poster) }}" alt="poster">
                    </div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('books.edit',$book->id) }}">
                         ...
                         <a href="{{ route('books.destroy',$book->id) }}" data-url="{{ route('books.destroy', $book->id) }}" class="deletebtn ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
        ...
        @include('backend.modals.delete',['name' => 'Book'])
    </div>
@endsection
